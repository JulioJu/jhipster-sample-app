...
type EntityResponseType = HttpResponse<Operation>;
type EntityArrayResponseType = HttpResponse<Operation[]>;
...
export class OperationService {
    ...
    create(operation: Operation): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(operation);
            return this.http.post<Operation>(this.resourceUrl,
copy,
                    { observe: 'response' })
.map((res: EntityResponseType) => this.convertDateFromServer(res));
    ...
    update(operation: Operation): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(operation);
            return this.http.put<Operation>(this.resourceUrl,
copy,
                    { observe: 'response' })
.map((res: EntityResponseType) => this.convertDateFromServer(res));
    }
...
.map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res));
    ...
    private convertDateFromClient(operation: Operation): Operation {
